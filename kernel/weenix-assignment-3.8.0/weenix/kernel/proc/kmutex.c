...
kmutex_lock(kmutex_t *mtx)
{
		...
		//dbg(DBG_PRINT, "(GRADING1A)\n");
		// At this point, lock must have been acquired by the current thread
...
kmutex_lock_cancellable(kmutex_t *mtx)
{
        ...
        // if need to wait
        if (mtx->km_holder != NULL){
			// if thread is cancelled during wait, must return -EINTR 
			// and must not hold any lock upon return
			if (sched_cancellable_sleep_on(&mtx->km_waitq)==-EINTR){
				// acquired lock when being cancelled, which is not 
				// possible in the current test suite, so the code below is
				// is not included for grading purposes
			...
			// finished waiting at this point with lock in hand
			//dbg(DBG_PRINT, "(GRADING1C)\n");
		}
		// At this point, lock must havebeen acquired by the current thread
		mtx->km_holder = curthr;
		//dbg(DBG_PRINT, "(GRADING1C)\n");
		return 0;
}
