...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
		...
		context_setup(&kt->kt_ctx, func, arg1, arg2, kstack, DEFAULT_STACK_SIZE, p->p_pagedir);
		kt->kt_kstack = kstack;
		kt->kt_proc = p;
		kt->kt_state = KT_RUN;
		kt->kt_cancelled = 0;
...
{
    // cleanup thread control block
	kthread_t* prev = curthr;
	prev->kt_retval = retval;
	prev->kt_state = KT_EXITED;
    ...
    // cleanup the current thread and yield CPU to another thread
    // so now the thread is simply in zombie state
...
kthread_clone(kthread_t *thr)
{
        ...
        char* kstack = alloc_stack();
        kthread_t* ck = (kthread_t*) slab_obj_alloc(kthread_allocator);
		//context_setup(&kt->kt_ctx, func, arg1, arg2, kstack, PAGE_SIZE, p->p_pagedir);
		ck->kt_kstack = kstack;
		ck->kt_state = KT_RUN;
		ck->kt_cancelled = 0;
		ck->kt_wchan = NULL;
		...
		ck->kt_ctx.c_eip = thr->kt_ctx.c_eip;
		ck->kt_ctx.c_pdptr = thr->kt_ctx.c_pdptr;
		ck->kt_ctx.c_esp = thr->kt_ctx.c_esp;
		//not too sure about it ...
		ck->kt_ctx.c_kstack = (uintptr_t)kstack;//thr->kt_ctx.c_kstack;
		ck->kt_ctx.c_kstacksz = thr->kt_ctx.c_kstacksz;
		KASSERT(KT_RUN == ck->kt_state); /* new thread starts in the runnable state */
        dbg(DBG_PRINT, "(GRADING3A 8.a)\n");
