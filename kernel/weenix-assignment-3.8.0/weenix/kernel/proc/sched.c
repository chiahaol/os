...
sched_cancel(struct kthread *kthr)
{
	...
	if (kthr->kt_state == KT_SLEEP_CANCELLABLE){
		// remove it from sleeping queue and move it to runq, so that it can be cancelled promptly
 ...
 */
//we implemented just follow what slides 14-2 said
void
sched_switch(void)
{
	...
	while(sched_queue_empty(&kt_runq)) {
		...
		intr_setipl(IPL_LOW); // unmask all interrupts
		intr_disable();
		if (sched_queue_empty(&kt_runq)){
			intr_wait(); // enable interrupt & halt CPU this is for kernel assignment 3 usage it also need to add more code since it has bug in it
		}
		intr_setipl(IPL_HIGH);
		//dbg(DBG_PRINT, "(GRADING1A)\n");
	}
	...
	curproc = curthr->kt_proc;
        kthread_t * localCurThr = curthr;
	curproc->p_state = PROC_RUNNING;
	//dbg(DBG_PRINT, "(GRADING1A)\n");

  	context_switch(&OldThread->kt_ctx, &curthr->kt_ctx);
          
        // switch back global variables 
        curthr = OldThread;
	curproc = OldThread->kt_proc;
        OldThread->kt_state = KT_RUN;
  	intr_setipl(oldIPL);
	  //dbg(DBG_PRINT, "(GRADING1A)\n");
}
