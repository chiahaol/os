...
do_read(int fd, void *buf, size_t nbytes)
{
	...
	dbg(DBG_TEMP, "Attempting to read file (fd = %d)\n", fd);
	if(fd < 0 || fd >= NFILES) {
...
do_write(int fd, const void *buf, size_t nbytes)
{
	...
	dbg(DBG_TEMP, "Attempting to write file (fd = %d)\n", fd);
	if(fd < 0 || fd >= NFILES) {
		// dbg(DBG_PRINT, "(GRADING2B)\n");
		return -EBADF;
	}
...
do_close(int fd)
{
	...
	dbg(DBG_TEMP, "Attempting to close file (fd = %d)\n", fd);
	if(fd < 0 || fd >= NFILES) {
		// dbg(DBG_PRINT, "(GRADING2B)\n");
		return -EBADF;
	}
	...
	// fput() twice because we call fget() in the beginning
	fput(f);
	fput(f);
...
do_mknod(const char *path, int mode, unsigned devid)
{
		...
		int ret = dir_namev(path, &namelen, &name, curproc->p_cwd, &dir_vnode);
		// if (ret != 0 ) {
		// 	// dbg(DBG_PRINT, "(GRADING2)\n");
		// 	return ret;
		// }
		// if (lookup(dir_vnode, name, namelen, &target_vnode) == 0) {
		// 	vput(dir_vnode);
		// 	vput(target_vnode);
		// 	// dbg(DBG_PRINT, "(GRADING2)\n");
		// 	return -EEXIST;
...
do_mkdir(const char *path)
{
	...
	vnode_t* target_vnode = NULL;
	int ret;
	if ((ret = dir_namev(path, &namelen, &name, curproc->p_cwd, &dir_vnode)) != 0 ) {
		// dbg(DBG_PRINT, "(GRADING2B)\n");
		return ret;
	}
	if (lookup(dir_vnode, name, namelen, &target_vnode) == 0) {
...
do_unlink(const char *path)
{
	...
	if ((ret = lookup(dir_vnode, name, namelen, &target_vnode)) < 0) {
		vput(dir_vnode);
		// dbg(DBG_PRINT, "(GRADING2B)\n");
		return ret;
	}
...
do_link(const char *from, const char *to)
{
	...
	vnode_t* dir_vnode = NULL;
	if ((ret = open_namev(from, O_RDONLY, &from_vnode, curproc->p_cwd)) < 0) {
		// dbg(DBG_PRINT, "(GRADING2B)\n");
		return ret;
	}
	...
	if ((ret = dir_namev(to, &namelen, &name, curproc->p_cwd, &dir_vnode)) != 0 ) {
		vput(from_vnode);
		return ret;
	}
	if (lookup(dir_vnode, name, namelen, &to_vnode) == 0) {
		vput(from_vnode);
		vput(to_vnode);
		vput(dir_vnode);
		return -EEXIST;
...
do_stat(const char *path, struct stat *buf)
{
	...
	// dbg(DBG_PRINT, "(GRADING2B)\n");
	ret = target_vnode->vn_ops->stat(target_vnode, buf);
	vput(dir_vnode);
	vput(target_vnode);
	// dbg(DBG_PRINT, "(GRADING2B)\n");
	return ret;
