...
sys_read(read_args_t *arg)
{
        ...
        void *allocated_kern_buf = page_alloc();
        size_t tot_nbytes_read = 0;
        // NOT SURE IF BELOW using while look IS RIGHT
        // write PAGE_SIZE at a time, since kern_args->nbytes might be > PAGE_SIZE
        // and our allocated_kern_buf will not be large enough
        // while (tot_nbytes_read < kern_args.nbytes){
        size_t num_bytes_to_read = PAGE_SIZE;
        if (kern_args.nbytes - tot_nbytes_read < PAGE_SIZE){
                num_bytes_to_read = kern_args.nbytes - tot_nbytes_read;
                dbg(DBG_PRINT, "(GRADING3D)\n");
        }
        int num_bytes_read = do_read(kern_args.fd, allocated_kern_buf, num_bytes_to_read);
        if (num_bytes_read < 0) {
                curthr->kt_errno = -num_bytes_read;
                page_free(allocated_kern_buf);
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                return -1;
        }
        dbg(DBG_PRINT, "(GRADING3D 1)\n");
        copy_to_user(kern_args.buf, allocated_kern_buf, num_bytes_read);
        tot_nbytes_read += num_bytes_read;
        // move forward this pointer to read next page of data
        // kern_args.buf = (void*)((size_t)kern_args.buf + num_bytes_read);
        // }
        page_free(allocated_kern_buf);
...
sys_write(write_args_t *arg)
{
	...
	int err;
        ...
        void *allocated_kern_buf = page_alloc();
        size_t tot_nbytes_written = 0;
        while (tot_nbytes_written < kern_args.nbytes){
                size_t num_bytes_to_write = PAGE_SIZE;
                if (kern_args.nbytes - tot_nbytes_written < PAGE_SIZE){
                        num_bytes_to_write = kern_args.nbytes - tot_nbytes_written;
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                }
                // NOT SURE if below is needed in order to first copy data from 
                // kern_args.buf in user space to allocated_kern_buf in kernel space
                if ((err = copy_from_user(allocated_kern_buf, kern_args.buf, num_bytes_to_write)) < 0) {
                        curthr->kt_errno = -err;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");
                        return -1;
                }
                // File object tracks the cursor position for us as we continue to write
                // to the file, so we don't need to adjust position to write to
                int num_bytes_written = do_write(kern_args.fd, allocated_kern_buf, num_bytes_to_write);
                if (num_bytes_written < 0) {
                        curthr->kt_errno = -num_bytes_written;
                        page_free(allocated_kern_buf);
                        return -1;
                        dbg(DBG_PRINT, "(GRADING3D 1)\n");                        
                }
                // NOT SURE if copy_to_user is needed or not
                // copy_to_user(kern_args->buf, allocated_kern_buf, num_bytes_written)
                kern_args.buf = (void*)((size_t)kern_args.buf + num_bytes_written);
                tot_nbytes_written += num_bytes_written;
                dbg(DBG_PRINT, "(GRADING3D)\n");
        }
...
sys_getdents(getdents_args_t *arg)
{
        ...
        dirent_t *allocated_kern_buf = kmalloc(sizeof(dirent_t));
        size_t tot_cnt_read = 0;
        while (tot_cnt_read < kern_args.count) {
                err = do_getdent(kern_args.fd, allocated_kern_buf);
                if (err < 0) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        curthr->kt_errno = -err;
                        kfree(allocated_kern_buf);
                        return -1;
                } else if (err == 0) {
                ...
                dbg(DBG_PRINT, "(GRADING3B)\n");
                copy_to_user(kern_args.dirp, allocated_kern_buf, sizeof(dirent_t));
                kern_args.dirp = (void*)((size_t)kern_args.dirp + sizeof(dirent_t));
                tot_cnt_read += err;
                memset((void *) allocated_kern_buf, 0, sizeof(dirent_t));
        }
        kfree(allocated_kern_buf);
        dbg(DBG_PRINT, "(GRADING3B)\n");
        return tot_cnt_read;
