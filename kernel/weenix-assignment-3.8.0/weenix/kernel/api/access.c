...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t * tgt_vmarea = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr));
        if (tgt_vmarea == NULL) {
                dbg(DBG_PRINT, "GRADING3D\n");
                return 0;
        }
        
        // if perm need PROT_READ but prot does not have it
        if ((perm&PROT_READ)>0 && (tgt_vmarea->vma_prot&PROT_READ)==0){
                dbg(DBG_PRINT, "GRADING3D\n");
		return 0;
        }
        // if perm need PROT_WRITE but prot does not have it
        if ((perm&PROT_WRITE)>0 && (tgt_vmarea->vma_prot&PROT_WRITE)==0){
                dbg(DBG_PRINT, "GRADING3D\n");
		return 0;
        }
        // if perm need PROT_EXEC but prot does not have it
        if ((perm&PROT_EXEC)>0 && (tgt_vmarea->vma_prot&PROT_EXEC)==0){
                dbg(DBG_PRINT, "GRADING3D\n");
		return 0;
        }
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        uint32_t lopage = ADDR_TO_PN(avaddr);
        uint32_t hipage = ADDR_TO_PN((size_t) avaddr + len);
        for (uint32_t vfn = lopage; vfn <= hipage; vfn++) {
                if (addr_perm(p, (const void *) PN_TO_ADDR(vfn), perm) == 0) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                        return 0;
                }
                dbg(DBG_PRINT, "(GRADING3A)\n");
